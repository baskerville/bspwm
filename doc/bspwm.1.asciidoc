...
FLAG := hidden|sticky|private|locked|overlay|urgent
...
[!](hidden|sticky|private|locked|overlay|urgent)::
...
overlay::
	Pass input to the window below.
...
*-g*, *--flag* hidden|sticky|private|locked|overlay[=on|off]::
...
*-a*, *--add* (<class_name>|\*)[:(<instance_name>|\*)] [*-o*|*--one-shot*] [monitor=MONITOR_SEL|desktop=DESKTOP_SEL|node=NODE_SEL] [state=STATE] [layer=LAYER] [split_dir=DIR] [split_ratio=RATIO] [(hidden|sticky|private|locked|overlay|center|follow|manage|focus|border)=(on|off)] [rectangle=WxH+X+Y]::
	Create a new rule.
...
'node_flag <monitor_id> <desktop_id> <node_id> hidden|sticky|private|locked|overlay|urgent on|off'::
