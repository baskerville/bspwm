...
*-l*, *--layout* 'CYCLE_DIR'|tiled|stacked|monocle::
...
*-D*, *--stacked-dir* 'DIR'::
	Set stacked layout direction.

*-R*, *--stacked-ratio* 'RATIO'|(+|-)('PIXELS'|'FRACTION')::
	Set the stacked ratio of the selected desktop (0 < 'RATIO' < 1).
...
'stacked_layout'::
	Display tiled windows in a master-stack layout.

'stacked_direction'::
	Desktop edge to use for master-stack.

'stacked_ratio'::
	Default window ratio for stacked mode.
...
'desktop_layout <monitor_id> <desktop_id> tiled|stacked|monocle'::
